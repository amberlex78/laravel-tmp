...
.PHONY: help setup build up down restart ps logs shell artisan composer npm key prepare-dependencies vendor-permissions migrate fresh test
...
prepare-dependencies: ## Create dependency directories and set host ownership
	$(COMPOSE) run --rm --no-deps --user root --entrypoint sh composer -lc 'mkdir -p /var/www/html/vendor /var/www/html/node_modules && chown -R $(HOST_UID):$(HOST_GID) /var/www/html/vendor /var/www/html/node_modules'

vendor-permissions: prepare-dependencies ## Keep the legacy permissions target available
